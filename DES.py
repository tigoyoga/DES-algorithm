...
def des_encrypt(plaintext, hex_key):
    ...
    debug_print("\nEncryption Input", f"Original Plaintext: {original_plaintext}")
    debug_print("Padded Plaintext", padded_plaintext)
    debug_print("\nHexadecimal Key (64-bit)", hex_key)
    debug_print("Binary Key for DES (64-bit)", binary_key)
    ...
    debug_print("\nSubkey Generation", "\n".join(f"Subkey {i+1}: {bin_to_hex(subkey)}" for i, subkey in enumerate(subkeys)))
...
def des_decrypt(ciphertext, hex_key):
    ...
    debug_print("\nDecryption Input", f"Ciphertext (hex): {binascii.hexlify(ciphertext.encode()).decode()}")
    debug_print("Hexadecimal Key (64-bit)", hex_key)
    debug_print("Binary Key for DES (64-bit)", binary_key)
...
ciphertext = des_encrypt(plaintext, hex_key)

decrypted = des_decrypt(ciphertext, hex_key)
debug_print("\nFinal Decrypted Text", decrypted)

assert plaintext == decrypted, "Dekripsi gagal"
debug_print("Result", "Enkripsi dan dekripsi berhasil!")